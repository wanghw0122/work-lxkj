...
public interface RcQuickChatUserPoolMapper {
    ...
    @Insert({ "insert into rc_quick_chat_user (user_id, pool_id, channel, vip_level, " +
            "gender, country_id, eroticism_behavior,head_img,user_name,age,app_id,stone_version," +
            "status,extend,platform_type) values (#{userId}, #{poolId}, #{channel}, #{vipLevel}, #{gender},#{countryId}," +
            "#{eroticismBehavior},#{headImg},#{userName},#{age},#{appId},#{stoneVersion}," +
            "#{status},#{extend},#{platformType})" })
